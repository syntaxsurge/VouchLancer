...
export async function seedUserTeam() {
  ...
  for (const { name, email, role } of SEED) {
    ...
    if (!u) {
      ;[u] = await db
        .insert(users)
        .values({ name, email: lowerEmail, role, passwordHash })
        .returning()
      ...
      if (!u.passwordHash) updates.passwordHash = passwordHash
  ...
  if (existingPipes.length === 0) {
    const PIPELINES = [
      ...
      },
      {
        ...
        description: 'Design cost-effective, resilient cloud architectures on cheqd.',
      },
    ]
